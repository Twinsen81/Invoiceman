...
class InvoicesInProgressFragment : MviFragment<InvoicesUiState, InvoicesUiEffect, InvoicesEvent>() {
    ...
    override fun onRenderUiState(uiState: InvoicesUiState) {
        ...
        if (uiState.invoices.isNotEmpty())
            listOfInvoices.onNext(uiState.invoices.filter { it.processedByUser == sessionManager.currentUser.id })
...
*/
    ...
    override fun getUiEffectObservable(): Observable<InvoicesUiEffect>? = null//viewModel.uiEffects
